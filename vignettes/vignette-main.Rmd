...
For a detailed walk-through of this example, see [here](quick_start_example1.html)
...
The type of data for which COLONY is most suitable is a single cohort with putative full- and half-siblings, with none of the parents genotyped. When running the same dataset with `sequoia` and default parameter settings, the results will typically differ, for the following reasons:

- COLONY only considers full sibling, half sibling, or unrelated. `sequoia` also considers full- or half- avuncular, grand-parental, etc., unless you specify that all individuals are from a single cohort, i.e. all individuals have the exact same birth year. 
- `sequoia` by default also considers double relationships, e.g. paternal half-siblings that are also maternal half-cousins, with a relatedness inbetween full and half siblings. To ignore these possibilities, use parameter `Complex='simp'` when running `sequoia()`. 
- When no parents of known sex can be assigned, there is in `sequoia` no way of differentiating between maternal and paternal half-siblings, and none are assigned (although they can be found with `GetMaybeRel()`. In COLONY candidate mothers and fathers can be specified, but this option is not available in `sequoia` (yet?). 
