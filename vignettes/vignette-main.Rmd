...
For a detailed walk-through of this example, see the [worked examples](./Worked_examples.html)
...
### Effect 
A higher rate of genotyping errors leads to more false negatives and more false positives during pedigree reconstruction, as illustrated in Figure \@ref(fig:ErrSimSeq). The magnitude of the effect is unpredictable: One wrong assignment due to a few genotyping errors in a key individual may have numerous knock-on effects, while dozens of genotyping errors in 'dead end' individuals may have no consequences at all. 

The simulations (`SimGeno()`) assume amongst others that genotyping errors are independent from each other and from a sample's call rate, and are likely give a too optimistic picture.


### Assumed genotyping error rate `Err`
Assignment success is reduced when the presumed genotyping error rate `Err` differs greatly from the actual genotyping error rate. Among others, when `Err` is much lower than the actual genotyping error rate (Figure \@ref(fig:ErrSimSeq): yellow symbols at right-hand side), some true parent-offspring pairs will be assigned as full siblings. In contrast, when `Err` is much too high (Figure \@ref(fig:ErrSimSeq): red symbols at keft-hand side) some true full siblings will be classified as parent-offspring. The consequences of being off by a factor 2 are limited, but being off by a factor 10 or even 100 may affect assignment considerably.

Computational time tends to increases with both simulated and assumed genotyping error rate, as the time-saving filtering steps become less effective. 
...
You can find extensive details about the ageprior [in this separate vignette](./introduction.html), including worked examples, mathematical framework, and full range of options.
...
Pedigree reconstruction with `sequoia()` is primarily developed for situations were false positive assignments have more severe downstream consequences than false negatives, and is by design very conservative when making assignments. 

There are four main causes for false negatives, which are not mutually exclusive:
...
2. The relationship is clear (e.g. half siblings), but it is unclear whether they are maternal or paternal relatives;
3. It is clear the pair are parent and offspring (or grandparent -- grand-offspring), but it is unclear which of the two is the (grand)parent, due to unknown birth year(s) and absence of a 'complementary' candidate parent;
4. There is more than one plausible candidate (dummy) (grand)parents of one sex, all of which are more likely to be a parent than otherwise related. This includes the situation where true offspring are among the candidate parents due to unknown birth years. 


Cause (1) may be due to having few, not-very-informative and/or high-error rate SNPs, making it hard to distinguish between parents and full siblings, or between full and half siblings. Lowering the assignment threshold `Tassign` may reduce this problem, but with imperfect genetic data some full siblings may 'look' (slightly) more like parents than like full siblings, resulting in erroneous assignments or false negatives via the 4th route. 

Even with perfect genetic data, half-siblings, grandparent--grand-offspring and full avuncular pairs are genetically indistinguishable, unless both already have one parent assigned. Assignment of half-sibling pairs is only possible when grand-parental and avuncular relationships can be excluded based on the age-difference of the pair, or when both individuals already have a parent assigned. 

Functions `GetMaybeRel()` and/or `CalcPairLL()` may help track down the main cause(s) for non-assignment. Many low LLRs (small differences between likelihoods) indicate too limited genetic information to tell the relationships apart, while many pairs indicated as '2nd' suggest it is not possible to tell the different 2nd degree relationships apart due to inconclusive age data and/or too few genotyped parents. 

Adding an individual to a sibship cluster is 'easier' than assigning a pair of half-siblings. The larger the sibship is, the more obvious it becomes genetically whether the individual is related to all of the siblings or not, and, with overlapping generations, the stronger the age differences point toward half-sibling, full avuncular, or grandparent. 

Moreover, maternal sibships help cluster paternal sibships, and vice versa. For example, individuals A and B are assigned as paternal half-siblings, because both each first had a genotyped mother assigned. Individual C is added, as it is related to both A and B and age-wise cannot be avuncular or grandparent. Individual D was similarly added to a different paternal sibship. If C and D are second degree relatives, they must be maternal relatives, and because both already have a (dummy) parent assigned the likelihood can tell us whether they are half-siblings, avuncular, or grandparent -- grand-offspring. 

But in the algorithm used, for sibship clusters to exist first sibling pairs must be assigned. This may not happen at all, or only very sporadically, when e.g. almost all birth years are unknown, or when no candidate parents were genotyped and maternal and paternal half-siblings cannot be distinguished based on age difference. Then these cascading assignments never kick off, and assignment rate can be extremely low. For such situations, different programs with MCMC based approaches are much more suitable. 



## Parent LLR 
When parentage assignment or pedigree reconstruction is completed (when the total likelihood has asymptoted) for each assigned parent-offspring pair a log10-likelihood ratio (LLR) is calculated. This is 
...
knitr::kable(cbind(" "= c("PO", "FS", "HS", "GP", "FA", "HA", "U"),
             ...
             booktabs=TRUE, position="!b")
```


The 'next-most-likely relationship' is one of seven different relationships (Table \@ref(tab:rel7)). Under the default settings, this includes their combinations, e.g. a pair may be both paternal half-siblings and maternal aunt/niece (section [Unusual relationships](#sec:WeirdRels)). To reduce computational time, these seven likelihoods are only calculated after filtering pairs based on the number of [Mendelian errors](#sec:MendelErrors). In addition, each likelihood is only calculated if that particular relationship is possible based on the [age difference](#sec:BY-AP) of the pair. The seven underlying likelihoods for each pair can be retrieved with [`CalcPairLL()`](#sec:SevenLL). 


The returned parental LLR's are calculated conditional on the final pedigree, and this step can be quite time-consuming. Therefore `sequoia()` can be run with `CalcLLR=FALSE`, and `CalcOHLLR()` can be run separately later (or for any existing pedigree).
...
[^d]: The matrix returned by `DyadCompare` [Deprecated] is a subset of the matrix returned here using default settings.
...
* **`simp`**
Consider polygamous matings, but ignore all inbred and [double relationships](#sec:WeirdRels) (e.g. a pair being both paternal half-siblings and maternal full cousins). Such relationships may still be assigned as side-effect. This again can be useful if the SNP panel has limited power, and the occurance of such complex relationships is very rare.
* **`full`**
The default, do consider all kinds of [relationship combinations](#sec:WeirdRels) (e.g. a pair is both paternal half-siblings and maternal aunt-nephew, resulting from a mother and daughter mating mating with the same male).
...
```{r griffin-useAge, echo=TRUE, eval=TRUE}
...
```{r AP-2, eval=TRUE, out.width="70%", fig.align="center"}
...
```{r eval=TRUE, fig.cap="Relatedness hexbinplot example"}
...
## Seven likelihoods  {#sec:SevenLL}
If you wish to see the likelihoods underlying the likelihood ratios reported in the pedigree or the `GetMaybeRel()` output, `CalcPairLL()` is available. You can specify a pedigree and/or ageprior to condition on, and can include a pair multiple times with different age difference or focal relationship to explore their effects on the likelihoods and thereby the LLR. 

### Parental LLR
The likelihood ratio as reported in the pedigree for a parent -- offspring pair $P$, $O$ is calculated as follows:

* temporarily drop both parents of $O$
* calculate the likelihoods for the various possible relationships between $P$ and $O$
* calculate single parent LLR: $LLR = LL_{PO} - LL_{next-most-likely}$
* restore the co-parent of $P$, if any
* again calculate the likelihoods for the various possible relationships between $P$ and $O$ 
* calculate parent-pair LLR: $LLR = LL_{PO} - LL_{next-most-likely}$, where $LL_{PO}$ is conditional on $P$'s co-parent, and $LL_{next-most-likely}$ is with or without the co-parent (typically withs are likelier than withouts, if the parent pair was assigned)
* restore both parents of $O$

In `CalcPairLL()`, you can specify to temporarily drop one or both parents via the `dropPar1` column in the `Pairs` dataframe.  



## Estimated genotype probability 
The actual genotype of individuals is estimated from their observed genotype, the presumed genotyping error rate and pattern, and the (estimated) genotype of any assigned parents. This is used to fill in any missing genotyped, and to allow and possibly adjust for any genotyping errors. It can be returned as a 3D array by `CalcOHLLR()`. 

During pedigree reconstruction, updating both pedigree links and genotype estimates risks wild oscillations if the latter are based on both parental and offspring genotypes. Therefore, offspring genotypes do currently not contribute to the genotype estimates for real individuals. For dummy individuals, it is context-dependent whether (part of) the offspring contribute. 

A more consistent implementation of real and dummy individuals is hoped for
in a future version, but would require rewriting a large part of the Fortran
source code.
...
## Hermaphrodites
Hermaphrodites have been re-implemented in version 2.2, and now full pedigree reconstruction should work largely as expected. 

A challenge with hermaphrodites is that additional information is required to determine whether a genetic parent should be assigned as dam or as sire. Sex-linked markers are not implemented in `sequoia`, and sequential hermaphroditism (male until age $t$, and female after, or *v.v.*) is not the norm. 

Two alternative approaches are possible: provide a pedigree prior, or ignore sex roles altogether. 

### Pedigree prior
This is a dataframe with for each individual its candidate dam and/or ('pedigree prior'), e.g. the plant from which the seed was collected. Then, when two configurations are genetically equally likely, the one that matches the prior pedigree is assigned. Candidate parent--offspring pairs that are not genetically a match will never be assigned.

This pedigree 'prior' can be provided by specifying `SeqList$PedigreePar` as input *and* setting `Module='par'` (`MaxSibIter=0`):
```{r }
...
### Ignore sex role
In some settings it is irrelevant whether the parent was the maternal or paternal parent. When running `sequoia()` with `Herm='B'`, during `Module='par'` single parents are assigned as dams, and in parent-pairs dam vs sire is determined by their order in the genotype file. Half-sib clusters will therefore (initially) mostly appear as paternal siblings; when `Herm='B'` this has absolutely no biological meaning like pollen-donors being less likely to be sampled than ovule-donors (for this distinction, you need to run with `Herm='A'`). 

The offspring of a non-genotyped parent may be arbitrarily split into a paternal and a maternal sibship. These female/male 'clones' are currently not linked during reconstruction, but likely pairs are returned in the output as list element `DummyClones`. These also include cases of selfing by non-genotyped individuals, for both `Herm='A'` and `Herm='B'` (these may include sibships of size 1 without grandparents). 


### Assignment errors
`PedCompare()` is not useful in combination with `Herm='B'`, as any dam assigned as sire and v.v. will show up as error. Use instead `ComparePairs(, patmat=FALSE)`.

One type of assignment error is unavoidable: if the true parent is unsampled and the product of selfing, the true, sampled grandparent will be indistinguishable from a parent. If both the true parent and true grandparent are sampled, the inbred one will be assigned (which works as long as the grandparent is not also inbred). 
...
### Genotyping errors
A few SNPs with a high (apparent) error rate may throw off pedigree reconstruction if their erroneous signal is not sufficiently offset out by a large number of more accurate SNPs. Such SNPs can be identified with `SnpStats()`, using an existing pedigree or one from an preliminary round of parentage assignment. If there are clear outliers with regards to estimated error rate, it may be worth exploring pedigree reconstruction without these SNPs.
