...
#' @return A text file with the probability ratio of two individuals being a
...
MakeAgeprior <- function(UseParents = FALSE) {
  Specs <- ReadSpecs()
  nAgeClasses <- as.numeric(as.character(Specs["nAgeClasses", ]))
  if (UseParents) {
    Parents <- ReadTable(Specs["ParentageFileName", ], sep="")
  ...
  if (nAgeClasses > 1) {
    if (!UseParents | PropAss < 0.25) {   # rather arbitrary threshold.
        ...
        utils::write.table(AP, Specs["AgePriorFileName", ], row.names=FALSE, sep=" ", quote=FALSE)
    } else {
      MkAgePrior(LifeHistFile = Specs["LifehistoryFilename", ],
                 ParentageFile = Specs["ParentageFileName", ],
                 MaxT = FacToNum(Specs["nAgeClasses", ])-1,
                 AgePriorFile = Specs["AgePriorFileName", ])
    }
  } else {
...
MkAgePrior <- function(LifeHistFile = NULL,
                       ParentageFile = NULL,
                       MaxT = NULL,
                       AgePriorFile) {
LHData <- utils::read.table(LifeHistFile, header=T,
                     na.strings=c("","-9"), stringsAsFactors=FALSE)
names(LHData) <- c("ID", "Sex", "BY")
Par <- utils::read.table(ParentageFile, header=T)[,1:3]
...
utils::write.table(AgePrior, AgePriorFile, row.names=F, quote=F,sep=" ")
}
