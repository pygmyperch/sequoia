...
#'   & ME.
#' @param GenoProb  \code{TRUE/FALSE}, return NxLx3 array with probability
                      ...
                      quiet = FALSE)
{
  ...
  if (is.logical(AgePrior) && AgePrior %in% c(TRUE, FALSE)) {  # not: NA
    if (AgePrior & (CalcLLR | GenoProb)) {  # if !CalcLLR, AgePrior not used at all.
  ...
  if ("Specs" %in% names(SeqList)) {
    PARAM <- SpecsToParam(SeqList$Specs, SeqList$ErrM, ErrFlavour,
                          dimGeno = dim(GenoM), CalcLLR, GenoProb, quiet)
...
#                  calcllr = as.integer(CalcLLR),   # mv to SpecsIntMkPed
                  returngenop = as.integer(GenoProb),
                  ...
                  genoprob = double(2*Ng*ncol(GenoM)*3) )
  ...
  # GenoProb ----
  if (GenoProb) {
    GenoProbA <- array(TMP$genoprob, dim=c(3, ncol(GenoM), 2*Ng))
    GenoProbA <- aperm(GenoProbA, c(3,2,1))  # reshape to ind - snp - allele array
    # remove rows for non-existing dummies:
    keep <- 1:Ng
    if (PedN$Nd[1] >0)  keep <- c(keep, seq(Ng+ 1, Ng+ PedN$Nd[1]))
    if (PedN$Nd[2] >0)  keep <- c(keep, seq(Ng+ Ng/2 +1, Ng+ Ng/2 + PedN$Nd[2]))
    GenoProbA <- GenoProbA[keep, , ]
    dimnames(GenoProbA) <- list(id = c(gID,
                                       PedN$Renamed$dam[,"name"],
                                       PedN$Renamed$sire[,"name"]),
                                snp = seq_len(ncol(GenoM)),
                                x = c(0,1,2))
    return( list(Pedigree = Ped, GenoProbA = GenoProbA) )
  } else {
