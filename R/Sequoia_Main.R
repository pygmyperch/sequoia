...
#' 95\% confidence interval), number of offspring, and offspring IDs. From
#' version 2.1 onwards, this includes dummy offspring.}
...
#'   visualise results, \code{\link{GetRelM}} to turn a pedigree into pairwise
#'   relationships, \code{\link{CalcOHLLR}} to calculate OH and LLR and to
#'   retrieve estimates of the actual genotypes, \code{\link{CalcPairLL}} for
#'   the likelihoods underlying the LLR, \code{\link{PedCompare}} and
#'   \code{\link{ComparePairs}} to compare to a previous pedigree,
#'   \code{\link{EstConf}} and \code{\link{SimGeno}} to estimate assignment
#'   errors, \code{\link{writeSeq}} to save results, vignette("sequoia") for
#'   further details & FAQ.
#'
#' @examples
...
#' PC$Counts["GG",,]
#'
#' ComparePairs(Ped1=Ped_HSg5, Ped2=SeqOUT$PedigreePar)
#'
#' \donttest{
                    ...
                    Plot = NULL)
{
  ...
  if (any(LifeHistData$Sex==4) && PARAM$Herm == "no") {  #!grepl("herm", PARAM$Complex)) {
    if (!quietR) message("detected hermaphrodites (sex=4), changing Herm to 'A'")
#    PARAM$Complex <- "herm"
    PARAM$Herm <- "A"
  }
