...
#'   \code{InFile}. Ignored if package
#'   \code{\link[data.table:fread]{data.frable}} is present or if
...
#'   dataframe.
#'
#'   If package \code{{data.table}} is detected, \code{\link[data.table]{fread}}
...
GenoConvert <- function(InData = NULL,
  ...
  if (is.null(SNPnames)) {
    ...
    if (is.null(colnames(GenoTmp)) |
        all(colnames(GenoTmp) == paste0('V', seq(1:ncol(GenoTmp)))) |
        ncol(GenoTmp)!=nSnp) {
