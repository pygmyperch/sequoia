...
nRounds, nC(2), nAgeClasses, nPairs, BY1, maxAgePO, mxA, XP, Complx
...
double precision, allocatable, dimension(:,:,:,:,:) :: XPr
 ...
 character(len=200) :: ParentageFileName, PedigreeFileName, &
  AgePriorFileName
...
! check if all genotyped individuals have a gender in lifehistory data
do i=1,nInd
...
write (201, '(a15, a10, a30, a10, a30, a10)') "Type", "Row1", "ID1", &
  "Row2", "ID2", "nDiffer"
if (nDupGenoID>0) then
  do i=1,nDupGenoID
    write (201,'(a15, i10, " ", a30, i10, " ",a30)') "GenoID", & 
    dupGenoIDs(i,1), Id(dupGenoIDs(i,1)), dupGenoIDs(i,2), &
    Id(dupGenoIDs(i,2))
  enddo   
endif
if (nDupLhID>0) then
  do i=1,nDupLhID
    write (201,'(a15, i10," ", a30, i10, " ",a30)') "LifehistID",&
    DupLhIDs(i,1), NameLH(DupLhIDs(i,1)), DupLhIDs(i,2), &
    NameLH(DupLhIDs(i,2))
  enddo   
endif
if (nDupGenos>0) then
  do i=1,nDupGenos
    write (201,'(a15, i10," ",a30, i10," ",a30, i10)') "Genotype",&
    dupGenos(i,1), Id(dupGenos(i,1)), dupGenos(i,2), &
    Id(dupGenos(i,2)), nMismatch(i)  
  enddo   
endif
if (nSexless>0) then
  do i=1, nSexless
    write (201,'(a15, i10," ", a30, i10, " ",a30)') "NoSex", &
    Sexless(i), Id(Sexless(i)), 0, "NA"
  enddo
endif
 close (201)
...
! ####################################################################
subroutine parents(nParents, nAmbiguous, TotLL)
...
do Round=1,42
  call rchkusr()
  if (Round==42) call rwarn("Parentage not converging!")
  ...
  if (ABS(TotLL(Round + 1) - TotLL(Round)) < ABS(TF)) exit ! convergence
enddo
...
open (unit=201,file=trim(ParentageFileName), status="unknown") 
write (201, '(3a30, 5a12, 3a6)') "id", "dam", "sire", "LLR_dam", &
 "LLR_sire", "LLR_pair","OH_dam", "OH_sire", "RowO","RowD","RowS"   
do i=1,nInd
  write (201,'(3a30, 3f10.2, 2i12, 3i6)') Id(i), ParentName(i,1:2),&
  LR_parent(i,1:3), OppHomDF(i,1:2), i, Parent(i,1:2)
enddo   
 close (201)

! output non-assigned likely parents-offspring pairs. 
RelName = (/ "PO", "FS", "HS", "GP", "FA", "HA", "U ", "XX" /) 
open (unit=201,file="UnassignedParents.txt",status="unknown")
write (201, '(2a30, 2a5, 4a10, a5)') "ID1", "ID2", "Sex1", "Sex2",&
 "AgeDif", "BestRel", "LLR_R_U", "LLR_R1_R2", "OH"
do i=1,nInd-1  ! TODO: in separate subroutine
  do j=i+1,nInd
    if (OppHomM(i,j) > maxOppHom .or. OppHomM(i,j)<0) cycle
    if ((LLR_O(i,j)==999 .or. LLR_O(i,j)< 0) .and. &
      (LLR_O(j,i)==999 .or. LLR_O(j,i)< 0))  cycle
    if (ANY(Parent(i,:)==j) .or. ANY(Parent(j,:)==i)) cycle
    if (Parent(i,1)/=0 .and. Parent(i,2)/=0 .and. &
     Parent(i,1)==Parent(j,1) .and. Parent(i,2)==Parent(j,2)) cycle  
    if (Sex(j)/=3) then
    ...
    if (maybe == 1) then
      ...
      write (201,'(2a30, 2i5, i10, a10, 2f10.2, i5)') Id(i), &
        ID(j), Sex(i), Sex(j), AgeDiff(i,j), RelName(TopX),&
        LLR_O(i,j), dLL, OppHomM(i,j)
    endif
  enddo
enddo   
 close (201)
...
do Round=1, Nrounds
  ...
  if(Round > 1) then
    if (ABS(TotLL(Round-1) - TotLL(Round)) < ABS(TF)) then
      LastR = 1
    endif
  endif
...
!    open (unit=401,file="Sibships_R"//RoundC//".txt",status="replace")
...
!    close (401) 
  ...
  do k=1,2  ! give names to dummy parents
    do s=1, nC(k)
      write(DumTmp, '(i4.4)') s
  ...
  if (Round == nRounds .or. LastR==1) then
    ...
    write (407,'(3a30, 3a10)')  "id", "dam", "sire", "LLR_dam",&
    "LLR_sire", "LLR_pair"
    do i=1,nInd
      write (407,'(3a30, 3f10.2)') Id(i), ParentName(i,1:2),&
      LR_parent(i,:)
    enddo
    do k=1,2
      do s=1,nC(k)
        write (407,'(3a30, 3f10.2)') DumName(s,k), GpName(:, s,k),&
          LR_GP(:, s, k)
      enddo
    enddo
    close (407)
    ...
    RelName = (/ "PO", "FS", "HS", "GP", "FA", "HA", "U ", "XX" /) 
    open (unit=201, file="UnassignedSibs.txt", &
    status="unknown")
    write (201, '(2a15, 2a5, 4a10)') "ID1", "ID2", "Sex1", "Sex2",&
    "AgeD", "BestRel", "LLR_R_U", "LLR_R1_R2"
    do i=1,  nInd-1
      do j=i+1,nInd
        do k=1,2
          if (k==2 .and. topX==4 .and. Parent(i,1)==0 .and. &
          Parent(i,2)==0) cycle
        ...
        if (Parent(i,k)/=0 .and. Parent(i,k)==Parent(j,k)) cycle
         ...
         if (k==2 .and. Parent(i,1)==0 .and. Parent(j,1)==0) cycle 
          ...
          if (AgeDiff(i,j)==999 .or. AgeDiff(i,j)>=0) then
            call CalcPair(i, j, k, .FALSE., tmpLL, 4)  
          else
          ...
          if (topX>4) cycle
          write (201,'(2a30, 2i5, i10, a10, 2f10.2)') Id(i),&
          ID(j), Sex(i), Sex(j), &
...
!   endif
  ...
  if (ALL(Parent(i,:)==0) .and. ALL(nCP>0)) then ! test combo's
    do u = 1, nCP(1)
...
if (AgeDiff(A,B)==999) then  
  do m=1,2
    if ((Sex(A)==m .or. Sex(A)==3) .and. Parent(B,m) < 0) then
      ...
      if (LLtmp(1)<0 .and. (LLtmp(1) - LLtmp(2)) > TF) then 
...
subroutine CalcPair(A, B, k, InclAge, LL, focal) 
...
if (LLg(1)==999)  call PairPO(A, B, k, LLg(1))
...
if (Complx==2 .and. LL(2)<0 .and. (focal==2 .or. focal==3) .and. &
  ...
  if (LL(2)/=222 .and. LLg(4)/=777 .and. (Parent(A,3-k)==0 .or. &
    ...
    if ((LLX(3)<0 .or. LLX(4)<0) .and. (LL(2) -MaxLL(LLX(3:4)))<TA) then
      LL(2) = 222
    endif
  endif
...
if (Parent(A,1)==Parent(B,1) .and. Parent(A,1)/=0 .and. &
  ...
  do k=1,2
    if (Parent(A,k) == B .or. Parent(B,k) == A) then
      LL = 777
      exit
    else if (Parent(A,k)/=Parent(B,k) .and. ((Parent(A,k)>0 .and. &
...
double precision, intent(OUT) :: LL
integer :: l, x, y, z,i, Par(2), GC(2)
double precision :: PrL(nSnp), PrXYZ(3,3,3), PrGA(3), PrGC(3,2)
    ...
    do x=1,3 
      do i=1,nS(SA,k)
        if (Genos(l, SibID(i,SA,k))/=-9) then
          PrXYZ(x,:,z) = PrXYZ(x,:,z) *OKA2P(Genos(l,SibID(i,SA,k)),x,z)
        endif
      enddo
    enddo
    do y=1,3 
  ...
  do k=1,2
    ...
    if (sx(1)==0 .and. sx(2)==0) then
      if (AgeDiff(ij(1),ij(2))==999) then
        ...
        call CalcPair(ij(1), ij(2), k, LR>0, LLtmp, 3)
      else
        call CalcPair(ij(2), ij(1), k, LR>0, LLtmp, 3)
      endif
      call BestRel(LLtmp, 2, topX, dLL)
      if (topX==2 .and. LR<0 .and. dLL < 2*TA) cycle  ! wait
      if (Complx>0)  call BestRel(LLtmp, 3, topX, dLL)
      if (.not. (topX==2 .or. topX==3)) cycle
      if (topX==3 .and. (PK==3 .or. (dLL<2*TA .and. LR<=0) .or. &
        Complx==0)) cycle  ! unclear if MHS or PHS
      ...
      Parent(ij(2), k) = -nC(k)
      call CalcCLL(nC(k), k)
      if (Complx==0 .and. Parent(ij(1), 3-k)==0) then  
        nC(3-k) = nC(3-k)+1  
        nS(nC(3-k), 3-k) = 2
      ...
      if (Complx==0) then  ! monogamy
        call BestRel(LLtmp, 2, topX, dLL) 
        if (topX==1 .and. dLL > TA * MIN(nS(sx(1),k), nS(sx(2),k))) then  
          call DoMerge(sx(1), sx(2), k)
          if (Parent(ij(1), 3-k)<0 .and. Parent(ij(2), 3-k)<0) then
            call DoMerge(-Parent(ij(1), 3-k), -Parent(ij(2), 3-k), 3-k)
          endif
        endif
      else
      ...
      do m=1,2
        if (sx(m)>0 .and. sx(3-m)==0) then
          if (Complx==0)  call CheckAdd(ij(3-m), sx(m), k, LLtmp, 2)
          if (Complx >0)  call CheckAdd(ij(3-m), sx(m), k, LLtmp, 3)
          call BestRel(LLtmp, 2, topX, dLL)
          if (topX/=2 .and. Parent(ij(3-m),3-k)==0 .and. &
            ALL(Parent(SibID(1:nS(sx(m),k),sx(m),k),3-k)==0)) cycle
          call BestRel(LLtmp, 3, topX, dLL)
          if (topX==2 .or. topX==3) then   ! *(nS(sx(m),k)+2)/2
            call DoAdd(ij(3-m), sx(m), k)
            if (Complx==0 .and. Parent(ij(m), 3-k)<0) then
...
do k=1,2
  ...
  do s=1,nC(k)-1
    ...
    do xr=s+1, nC(k)
      ...
      topX = 0
      call CheckMerge(s, r, k, k, LLm, 1)
      call BestRel(LLm, 1, topX, dLL)
      if (topX==1 .and. dLL > TA * MIN(nS(s,k), nS(r,k))) then 
        if (Complx==0 .and. Parent(SibID(1,s,k),3-k)<0 .and. &
          Parent(SibID(1,r,k),3-k)<0) then
          call DoMerge(s, r, k)
          call DoMerge(-Parent(SibID(1,s,k),3-k), &
            -Parent(SibID(1,r,k),3-k), 3-k)
        else
          call DoMerge(s, r, k)
        endif
        call CalcCLL(s,k)
...
do k=1,2   
  ...
  do i=1, nInd
    ...
    do s=1,nC(k)  ! sibships
      ...
      if (topX==2 .or. (topX==3 .and. Complx>0)) then 
        nMaybe = nMaybe+1
        Clm(nMaybe) = s 
        dLLM(nMaybe) = dLL
      endif
  ...
  do xs=1, CurNumC(k)
    ...
    do i=1,nInd
      ...
      do n=1,2
        ...
        if (AgeDiff(i,GpID(n,s,k)) < 0 .and. &
          AgeDiff(i,GpID(n,s,k))/= 999) then 
          maybe = 0
          exit
        endif
...
double precision :: LRG, ALRtmp, LL(7), LLG(21,21)
logical :: SexUnk(20,2)
...
do k=1,2
  ...
  do xs=1, nC(k)
      ...
      do r=1, nC(m) 
        ...
        if (ncG(m) < 20) then
          ...
          if (COUNT(nFS(SibID(1:nS(r,m),r,m))>0)==1 .and. &
            ALL(Parent(SibID(1:nS(r,m),r,m), 3-m)<=0)) &
              SexUnk(nCG(m),m) = .TRUE.
        endif                
      enddo
    enddo
    ...
    LLG = 999
    LLG(1,1) = CLL(s,k)  ! no GP
    if (ALL(nCG==1)) then
      cycle  ! no candidate GPs
    else if (ALL(nCG <= 2)) then
      if (ALL(nCG==2)) then
        GpID(1,s,k) = CandGP(2,1) 
        call CalcGPz(s, k, CandGP(2,2), 2)
        if (GpID(1,s,k)/=0) then  ! double check
          GpID(1,s,k) = 0
          call CalcGPz(s, k, CandGP(2,1), 1)
        endif
      else
        do m=1,2
          if (nCG(m)>1) then
            call CalcGPz(s, k, CandGP(2,m), m)
          endif
        enddo
      endif
    else  
      do u = 2, nCG(1)  ! test all combo's
        do v = 2, nCG(2)
          ...
          if (GpID(2,s,k)/=CandGP(v, 2)) then
            ...
            call CalcGPz(s, k, CandGP(v, 2), 2) 
          else
            call CalcGPz(s, k, CandGP(v, 1), 1) 
          endif
       if (GpID(1,s,k)==CandGP(u,1) .and. GpID(2,s,k)==CandGP(v,2)) then
        ...
        enddo
      enddo
      Best = MAXLOC(LLG, MASK=LLG<0)
      if (ALL(Best==1)) then
        GpID(:,s,k) = 0
      else 
        if (GpID(1,s,k)/=CandGP(Best(1),1)) then
          if (Best(1)==1)  GpID(1,s,k) = 0 
          if (Best(1) >1)  call CalcGpz(s,k, CandGP(Best(1),1), 1)
        endif
        if (GpID(2,s,k)/=CandGP(Best(2),2)) then
          if (Best(2)==1)  GpID(2,s,k) = 0 
          if (Best(2) >1)  call CalcGpz(s,k, CandGP(Best(2),2), 2)
        endif
        if (GpID(1,s,k)/=CandGP(Best(1),1) .and. Best(1) >1 .and. &
          GpID(2,s,k)==CandGP(Best(2),2)) then
          call CalcGpz(s,k, CandGP(Best(1),1), 1)
        endif
        if (Best(1) >1 .and. GpID(1,s,k)/=CandGP(Best(1),1) .and. &
          Best(2) >1 .and. GpID(2,s,k)/=CandGP(Best(2),2)) then
          GpID(1,s,k) = CandGP(Best(1),1)
          GpID(2,s,k) = CandGP(Best(2),2)
          call CalcCLl(s,k)
        endif
      endif
    endif
...
if (curGP(1)==0 .and. curGP(2)==0) then
  ...
  do m=1,2 ! sex currently assigned GP
    if (CurGP(m)==0) cycle        
    if (B > 0) then
      call checkAdd(B, SA, kA, LLA(m,:,4), 7)   ! CurGP(m)=GP + A_7 
    ...
    call CalcCLL(SA,kA)
    if (B > 0) then
      call checkAdd(B, SA, kA, LLA(m,:,7), 7)   ! A_7
    ...
    if (curGP(m) > 0) then
      call checkAdd(CurGP(m), SA,kA, LLA(m,7,:), 7)  ! CurGP(m)_7
    else if (curGP(m) < 0) then
    ...
    if (LLA(m,4,4)<0 .or. LLA(m,4,7)<0) then  ! Else not possible
      ...
      call CalcCLL(SA,kA)
      if (curGP(m) > 0) then
        call checkAdd(CurGP(m),SA,kA, LLA(m,4,:), 7) 
      else if (curGP(m) < 0) then
...
double precision :: LLg(7),  LLtmp(2,2), ALR(7), LLz(5), LRHS,&
   ALRx(2,2), LLM(3), LLp(7), LLx(3)
...
if (focal==3 .and. ((LL(2) > 0 .and. LL(3)>0) .or. &
 ...
 call CalcAgeLR(-SB,k, A,0, 0,1, ALR(4))  ! A parent of SB
if (LLg(4) < 0) then 
  if (ALR(4) /= 777) then
    LL(4) = LLg(4) + ALR(4)
  else
...
if (Complx>0)  LL(6) = MaxLL(RESHAPE(LLtmp, (/2*2/) ))
...
if (complx>0 .and. (MaxLL(LL)==LL(3) .or. MaxLL(LL)==LL(2)) .and. &
  ...
  if (MaybeOpp == 1 .and. Par < 0) then
    ...
    if (LLM(1) < 0 .and. (LLM(1) - LLM(2)) > 2*TA) then
      LL(2) = LL(2)   ! proceed.
    else if (LLM(2) < 0 .and. (LLM(2) - LLM(3)) > TA) then
...
if (kA == kB .and. focal==1 .and. (ABS(MaxLL(LL) - LL(1))<TA .or. &
  ...
  if (Complx==0)  LL(1) = LLM(4) ! + ALR(1)  # FS merge only
...
double precision :: PrL(nSnp, nS(SB,kB),3), PrY(3,3), PrX(3,3), PrZ(3),&
...
do f = 1, nS(SB, kB)
  if (MaybeFS(f)/=1) cycle
  dLL(f) = SUM(PrL(:, f,1)) - MAX(SUM(PrL(:,f,2)), SUM(PrL(:,f,3)))
enddo
...
integer, allocatable, dimension(:,:) :: GenosR
 character(len=2000) :: GenoFileName, LifehistFileName, dumC 
...
read (1,*) dumC, TF           
read (1,*) dumC, TA          ! 9    
read (1,*) dumC, ParentageFileName  
read (1,*) dumC, AgePriorFileName   ! 11
read (1,*) dumC, nAgeClasses
read (1,*) dumC, maxSibSize         ! 13
read (1,*) dumC, Nrounds
read (1,*) dumC, PedigreeFileName   ! 15
read (1,*) dumC, DumPrefix(1)    
read (1,*) dumC, DumPrefix(2)       ! 17 
read (1,*) dumC, Complx
 close (1)
...
read(103,*)   ! header
do i=1,nInd
  read(103,*) OffName(i), dumC(1:2), dumR(1:3), dumI(1:2), j,&
...
open (unit=301,file="DummyParents.txt",status="replace")
  write (301,'(3a20, a4, 3a8, 500a10)')  "id", "dam", "sire", "sex",&
   "est.BY", "min.BY", "max.BY", "NumOff", OffHeader
  do k=1,2
    do s=1,nC(k)
      write (301,'(3a20, i4, 4i8, " ", 500a10)') DumName(s,k),&
       GpName(:, s,k), k, EstDumBY(:, s, k), &
